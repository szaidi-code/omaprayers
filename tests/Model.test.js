...
test("method options follow catalog order and keep string values", () => {
  ...
  assert.equal(options.find(option => option.value === "24").label, "Astronomical Research Center (A.R.C.), Qom")
  assert.equal(Model.methodOptions("Arabic").find(option => option.value === "24").label, "مركز البحوث والدراسات الفلكية")
  assert.equal(Model.hanafiSupported(24), false)
  assert.equal(Model.hanafiSupported("0"), false)
